...
export default function AuditForm({ plan }: { plan: Plan | null }) {
  ...
  const maxMB    = plan === 'enterprise' ? 500 : plan === 'pro' ? 100 : 10
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (file.size > maxBytes) {
      setError(`Tu plan ${plan?.toUpperCase() ?? ''} permite archivos de maximo ${maxMB} MB.`)
      return
    }
    setLoading(true)
    setError(null)
  ...
  return (
    <div className="p-8">
      {/* Header */}
      <div className="mb-7">
        <h1 className="text-xl font-bold text-slate-900 tracking-tight">Nueva auditoria RIPS</h1>
        <p className="text-sm text-slate-500 mt-0.5">Resolucion 2275 de 2023 · Archivos .json, .rips o .zip</p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <form onSubmit={handleSubmit}>
                ...
                {file ? (
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">Archivos .json, .rips o .zip · max. {maxMB} MB</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            >
              ...
              {loading ? 'Analizando...' : 'Iniciar auditoria'}
            </button>
          </form>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            ...
            <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-md ${
              ...
              <span className={`w-1.5 h-1.5 rounded-full ${
                plan === 'enterprise' ? 'bg-purple-500' : plan === 'pro' ? 'bg-blue-500' : 'bg-slate-400'
              }`} />
              {plan?.toUpperCase() ?? 'FREE'}
            </span>
            <p className="text-xs text-slate-500 mt-2">
              Maximo <span className="font-semibold">{maxMB} MB</span> por archivo
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[11px] text-slate-400 uppercase font-semibold tracking-wide">Modulos validados</p>
            </div>
