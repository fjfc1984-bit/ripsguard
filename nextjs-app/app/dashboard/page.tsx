...
import { getSubscriptionStatus, type Plan } from '@/lib/subscription'
import {
    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <StatCard
          ...
          value={loading ? '—' : (stats?.auditorias_hoy ?? 0)}
          icon={SearchIcon} color="text-blue-600" bg="bg-blue-50"
        />
        <StatCard
          label="Este mes"
          value={loading ? '—' : (stats?.auditorias_mes ?? 0)}
          sub="auditorías completadas"
          icon={BarChartIcon} color="text-indigo-600" bg="bg-indigo-50"
        />
        <StatCard
          label="Críticos / mes"
          value={loading ? '—' : (stats?.total_criticos_mes ?? 0)}
          sub="hallazgos críticos"
          icon={AlertTriangleIcon} color="text-red-500" bg="bg-red-50"
        />
        <StatCard
          label="Valor en riesgo"
          value={loading ? '—' : formatCOP(stats?.valor_riesgo_mes ?? 0)}
          sub="posibles glosas este mes"
          icon={ZapIcon} color="text-amber-600" bg="bg-amber-50"
        />
      </div>
          ...
          {loading ? (
            ...
            <>
              <p className="text-sm font-bold text-slate-900">
                {ERROR_LABEL[stats.top_error] ?? stats.top_error}
              </p>
      ...
      <div className="bg-white rounded-xl border border-slate-200 mb-6">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <Link href="/dashboard/history" className="text-xs text-blue-600 hover:underline font-medium">
            Ver todo →
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div>
            {recent.map(item => <RecentAuditRow key={item.session_id} item={item}/>)}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <p className="text-xs text-slate-500 leading-relaxed">
            Todas tus auditorías anteriores con resultados y reportes descargables.
          </p>
